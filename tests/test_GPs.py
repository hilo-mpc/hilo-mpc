...
from hilo_mpc import GP, GPArray, Mean, Kernel
...
class TestGaussianProcessArray(TestCase):
    """"""
    def setUp(self) -> None:
        """
    ...
    def test_gaussian_process_array_length(self) -> None:
        """

        :return:
        """
        self.assertEqual(len(self.gps), 3)

    def test_gaussian_process_array_initialization(self) -> None:
        """
        ...
        features = [['a', 'b'], ['d', 'e'], ['g', 'h']]
        labels = [['c'], ['f'], ['i']]

        for k, gp in enumerate(gps):
            gp.initialize(features[k], labels[k])

        self.assertEqual(gps[0].features, features[0])
        self.assertEqual(gps[0].labels, labels[0])
        self.assertEqual(gps[1].features, features[1])
        self.assertEqual(gps[1].labels, labels[1])
        self.assertEqual(gps[2].features, features[2])
        self.assertEqual(gps[2].labels, labels[2])

    def test_gaussian_process_array_multiplication(self) -> None:
        """
        ...
        features = ['x', 'y', 'z']
        labels = ['sin_x', 'cos_y', 'tan_z']

        X_train = np.array([[0., .5, 1. / np.sqrt(2.), np.sqrt(3.) / 2., 1., 0.],
                            [1., np.sqrt(3.) / 2., 1. / np.sqrt(2.), .5, 0., -1.],
                            [0., 1. / np.sqrt(3.), 1., np.sqrt(3.), np.nan, 0.]])
        y_train = np.array([[0., np.pi / 6., np.pi / 4., np.pi / 3., np.pi / 2., np.pi]])

        for k, gp in enumerate(gps):
            gp.initialize(features[k], labels[k])
            if k == 0:
                gp.set_training_data(np.atleast_2d(X_train[k, :-1]), np.atleast_2d(y_train[0, :-1]))
            elif k == 2:
                gp.set_training_data(np.atleast_2d(X_train[k, :-2]), np.atleast_2d(y_train[0, :-2]))
            else:
                gp.set_training_data(np.atleast_2d(X_train[k, :]), y_train)
            gp.setup()
            gp.fit_model()

        X_sin = np.array([[.25, .6, .75, .9, .4]])
        y_sin, _ = gps[0].predict(X_sin)
        X_cos = np.array([[.9, .75, .6, .25, -.5]])
        y_cos, _ = gps[1].predict(X_cos)
        X_tan = np.array([[.3, .65, 1.4]])
        y_tan, _ = gps[2].predict(X_tan)

        B = np.array([[0., 2., 0.], [1., 0., 0.], [0., 0., .5]])
        gps_fun = B @ gps
        y = gps_fun(X_sin[:, :-2], X_cos[:, :-2], X_tan)

        np.testing.assert_allclose(y, np.concatenate([2. * y_cos[:, :-2], y_sin[:, :-2], y_tan / 2.]))
    ...
    def tearDown(self) -> None:
        ...
        if self.kernel is None:
            ...
            if self.kernel.acronym == 'Const':
                ...
                np.testing.assert_allclose([np.asarray(parameter.value).flatten()[0] for parameter in self.gp.hyperparameters][1:],
...
class RasmussenSimpleRegression(TestCase):
    ...
    def tearDown(self) -> None:
        ...
        np.testing.assert_allclose([np.asarray(parameter.value).flatten()[0] for parameter in self.gp_1.hyperparameters],
                                   np.array([.0222571, .3703166, 3.9427837]), rtol=1e-6)
        np.testing.assert_allclose([np.asarray(parameter.value).flatten()[0] for parameter in self.gp_2.hyperparameters],
        ...
        np.testing.assert_allclose([np.asarray(parameter.value).flatten()[0] for parameter in self.gp_3.hyperparameters],
